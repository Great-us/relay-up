...
你是安装器。**当前工作目录 = 目标项目根**。模板在本技能目录 `template/` 下。铁律：只写 `./relay/`、`./.zcode/skills/relay-next/`、`./tools/chat_send.py`、`./tools/verify_report.py`、`./relay/relay.enabled`；已存在的文件一律**不覆盖**（恢复语义）；不碰用户级配置、不碰凭据、不动项目其他文件；拿不准就列出计划先问。
...
- 未注册 hooks 时**不影响可用性**：手动 `/relay-next` 与员工值班 cron 自查两条路径完整可用（纯文件读写）。
- chat 双向、任务卡、验收（`tools/verify_report.py`）均为纯文件合同，天然跨项目。
